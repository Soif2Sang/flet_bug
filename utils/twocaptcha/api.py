        ...
        **kwargs : TYPE
        ...
        try:
            ...
            if files:
                ...
                resp = requests.post(current_url, data=kwargs, files=files)

                [f.close() for f in files.values()]

            elif "file" in kwargs:
                with open(kwargs.pop("file"), "rb") as f:
                    resp = requests.post(current_url, data=kwargs, files={"file": f})

            else:
                resp = requests.post(current_url, data=kwargs)
